...
**Multi-messenger consolidation in one interface:** Neither ALeRCE, Fink, nor Lasair natively combines optical transient alerts with gravitational wave events from the GWTC catalog and Fast Radio Bursts from CHIME/FRB in a single unified database with spatial cross-matching across all three messengers.
...
**PostGIS ST_DWithin over HEALPix:** Gravitational wave sky localization maps are natively represented as HEALPix probability distributions [@LVK2025]. Full HEALPix matching requires the `healpy` library, which has known installation issues on Windows development environments and adds significant complexity to deployment. Rubin Scout instead approximates the 90% credible region as a circle of equivalent area using $r = \sqrt{A_{90}/\pi}$, where $A_{90}$ is the credible region area in square degrees, and uses PostGIS `ST_DWithin` for spatial matching. This approximation introduces geometric error for highly non-circular skymaps (common in two-detector events) but provides exact results for well-localized events like GW170817 and is adequate for candidate shortlisting in follow-up planning workflows. Full HEALPix integration is documented as a future development target.
...
**GWOSC / GWTC:** Gravitational wave events are ingested from the GWOSC public catalog API, which provides all confirmed detections from the Gravitational Wave Transient Catalog (GWTC-1 through GWTC-4, 265 events) [@LVK2025]. GPS timestamps are converted to UTC using `astropy.time.Time` with `format='gps'` [@Astropy2022]. Merger type classification is inferred from component masses using the conventional 3 solar mass boundary between neutron stars and black holes.

**CHIME/FRB Catalog 1:** Fast Radio Burst detections are ingested from the CHIME/FRB Catalog 1 [@CHIMEFRB2021] via the CDS VizieR astronomical data service in VOTable format, parsed using `astropy.io.votable`. The catalog contains 600 FRB detections including dispersion measures, sky positions, and detection epochs.
...
The test suite comprises 35 unit tests across five modules, covering input validation (OID format patterns, classification allowlists), GW cross-matching logic (mass-based merger classification, great-circle separation), the ILMT recommendation engine (all three recommendation states), MJD-to-datetime conversion, and TNS data parsing (RA/Dec in both decimal and sexagesimal formats, TNS classification mapping, date parsing). Tests are implemented using pytest [@pytest] and can be run with:
